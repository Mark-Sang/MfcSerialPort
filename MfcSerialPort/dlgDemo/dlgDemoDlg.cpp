...
/////////////////////////////////////////////////////////////////////////////
// CDlgDemoDlg dialog
unsigned char CDlgDemoDlg::Globbuff[1024*768*4]={'\0'};
unsigned int CDlgDemoDlg::Wide=0;
unsigned int CDlgDemoDlg::Height=0;
unsigned int CDlgDemoDlg::Widebytes=0;
DWORD CDlgDemoDlg::Bufflength=0;
BITMAPFILEHEADER CDlgDemoDlg::bmpHeader; //文件头
BITMAPINFOHEADER CDlgDemoDlg::bmpInfo; 
...
BEGIN_MESSAGE_MAP(CDlgDemoDlg, CDialog)
	...
	ON_STN_CLICKED(IDC_ST_DISPLAY, &CDlgDemoDlg::OnStnClickedStDisplay)
	ON_BN_CLICKED(IDC_rolate, &CDlgDemoDlg::OnBnClickedrolate)
END_MESSAGE_MAP()
   ...
   if (DeviceInfo[dwCard].dwConnectState)
	{
			...
			DXConvertYUVtoRGB(DeviceInfo[dwCard].dwChannelHandle,
				...
				Wide=width;
				Height=height;
				Widebytes=bytesWidth;
				m_DeviceControl.ConstructBih(width,height,bmpInfo); 
				m_DeviceControl.ContructBhh(width,height, bmpHeader);
				DWORD dataBytes = bmpHeader.bfSize - bmpHeader.bfOffBits;
				Bufflength=dataBytes;
				memcpy(Globbuff,RgbBuf,dataBytes);
			  //定义变量存储图片信息
			  ...
			  memcpy(pBmpData,RgbBuf,dataBytes);

			  //显示图像
			  CWnd *pWnd=GetDlgItem(IDC_ST_DISPLAY); //获得pictrue控件窗口的句柄
			  CRect rect;
			  ...
			  //m_DeviceControl.StopPrview(DeviceInfo[0].dwChannelHandle);
...
void CDlgDemoDlg::OnStnClickedStDisplay()
{
	// TODO: 在此添加控件通知处理程序代码
}


void CDlgDemoDlg::OnBnClickedrolate()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	m_SerialPort.InitPort(this, 3, 9600, 'N', 8, 0);
	m_SerialPort.StartMonitoring();
	m_SerialPort.WriteToPort(a);
}
